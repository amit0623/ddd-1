	...
	DefaultKitchenService(EventLog eventLog, KitchenOrderRepository kitchenOrderRepository, PizzaRepository pizzaRepository, OrderingService orderingService) {
		...
		eventLog.subscribe(new Topic("kitchen_orders"), e-> {
			...
			if (e instanceof KitchenOrderPrepStartedEvent ) {
				KitchenOrderPrepStartedEvent kope = (KitchenOrderPrepStartedEvent) e ;
				createAndStartPrepOfKitchenOrderPizzas(kope);
			}
		...
		eventLog.subscribe(new Topic("pizzas"), e-> {
			...
			if (e instanceof PizzaPrepFinishedEvent ) {
				...
				KitchenOrder kitchenOder = kitchenOrderRepository.findByRef(pizza.getKitchenOrderRef());
				if (kitchenOder.isPrepping()) {
					kitchenOder.startBake();
				}
				...
				KitchenOrder kitchenOder = kitchenOrderRepository.findByRef(pizza.getKitchenOrderRef());
				if (kitchenOder.isBaking()) {
					kitchenOder.startAssembly();
				}
				if (pizzaRepository.findPizzasByKitchenOrderRef(kitchenOder.getRef())
						.stream()
						.allMatch(Pizza::hasFinishedBaking)
				) {
					kitchenOder.finishAssembly();
				}
			}
		});
	}
